...
# Print a Fernet key: 32 random bytes, urlsafe-base64 encoded. Generated by
# `cryptography` itself rather than `openssl rand -base64 32`, because Fernet
# wants the URL-SAFE alphabet and openssl emits the standard one — a key with a
# `/` in it is accepted by base64 and rejected by Fernet, at the first Dropbox
...
BOOT_POSTGRES_PASSWORD="$(rand_hex 16)" \
  ...
  BOOT_ENCRYPTION_KEY="$(fernet_key)" \
...
updates = {
    ...
    "SECRETS__ENCRYPTION_KEY": os.environ["BOOT_ENCRYPTION_KEY"],
...
echo "  SECRETS__ENCRYPTION_KEY    random Fernet key (encrypts stored cloud credentials)"
