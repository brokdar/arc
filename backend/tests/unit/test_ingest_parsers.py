...
from app.domain.streams import StreamChannel, channels_present, resample
...
def test_a_truncated_fit_file_is_unreadable_rather_than_silently_short(
    tmp_path: Path,
) -> None:
    # Half a real file: the strict SDK refuses it, and the tolerant reader is
    # asked next. Either it recovers records or the file is quarantined — what
    # must not happen is a session built from a header alone.
...
def test_extensions_are_normalised_for_dispatch_and_storage() -> None:
    ...
    assert extension_of(Path("noextension")) == ""
