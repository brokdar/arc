...
from app.domain.connections import AppKeySource, ConnectionProvider, ConnectionStatus
from app.persistence.connections import MAX_APP_KEY_LENGTH, MAX_REMOTE_PATH_LENGTH
...
class DropboxSetupRead(BaseModel):
    ...
    #: False until an app key exists in either source. The panel offers the
    #: registration checklist rather than a connect button that would fail.
    app_key_set: bool
    #: Which key is in force. `stored` is removable from the panel;
